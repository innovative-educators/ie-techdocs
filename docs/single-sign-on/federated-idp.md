...
If a learner with an account in a federated IdP moves between two or more schools that are both utilizing the IE platform, the IdP may send different a `studentLicenseId` depending on which integration the learner is authenticating to.
