...
1. `IdP XML Metadata file` This tends to be the easiest way to share the details required for SP configuration. If more convenient, you may provide these details individually instead.
...
1. The `SP Metadata XML file` which includes the relevant configuration details to use. Often IdP systems can import this file to save you the hassle of configuring manually.
  ...
  <details className={'alert alert--secondary'}>
    ...
    The easiest way to get these details is directly from the SP Metadata XML file you received from IE. If more convenient, you can request these details individually:
...
2. The `studentLicenseId` for your specific IE client. (this is a static attribute that IE will provide that needs to be passed for each user).
...
| `studentLicenseIds` | string | One or more license IDs to associate the content with the learner. This is a single value that must be sent for all learners and is best configured as a **static attribute**.&nbsp;&nbsp;[See below for more details](#studentlicenseid-as-a-static-attribute). <br/><br/>\* studentLicenseIds is required except in a single client instance (not common) | Required* |
...
For learners that should be associated with a client (almost every case), you must include one or more studentLicenseIds in the SAML attributes.

#### StudentLicenseId as a Static Attribute
To avoid the need for an extra column for every user in your system, you can utilize a `static attribute` to send over the required `studentLicenseIds` if your IdP allows configuring one. This is set up in your configuration as opposed to a table somewhere. The specific implementation will depend on your IdP, but below is an example in SimpleSAMLphp that may or may not be directly adaptable to your specific SAML IdP.

<details className={'alert alert--secondary'}>
<summary>A SimpleSAMLphp Example</summary>

This is a potential solution to sending the required studentLicenseId in the SAML payload without needing to adjust user records in SimpleSAMLphp.
...
From those docs, modify `saml20-sp-remote.php` and in the existing 'authproc' => array section (add it if missing), add static attributes using the core:AttributeAdd functionality like so:
...
All access restrictions, or lack thereof, are handled on the IdP side of things. So if applicable, make sure sufficient role permissions have been granted to the learners who'll need access to the IE platform before they need it.
...
## Authentication
Logins with IE are SP-initiated SSO with SAML, meaning that the login flow begins on the SP platform.
...
- Once a user visits this page, they will be redirected to your IdP for authentication. After authentication is successful, the IdP will send the user back to IE platform with a SAML Response that includes any attributes per the configuration. The user will then be logged into their SP client and will have access to the content that has been provisioned per the `studentLicenseId` configured in their attributes.
...
If authentication was successful, you should be redirected to the IE platform and logged in. If you are not logged in, and are still located on the IdP side, there's most likely an issue with the configuration on the IdP side.
